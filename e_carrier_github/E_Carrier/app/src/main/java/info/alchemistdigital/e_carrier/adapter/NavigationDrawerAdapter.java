...
public class NavigationDrawerAdapter extends RecyclerView.Adapter<NavigationDrawerAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        ...
        if( position == Integer.parseInt("2")){
            NavDrawerItem current = data.get(position);
            holder.title.setText(current.getTitle());
            holder.title.setTextColor(ContextCompat.getColor(context, R.color.colorPrimary));
            holder.title.setTypeface(null, Typeface.BOLD);
            holder.icon.setImageResource(R.drawable.ic_power_settings_new_black_24dp);
        }
        else {
            if( position == Integer.parseInt("0") ){
                holder.icon.setImageResource(R.drawable.ic_home_black_24dp);
            }
            if( position == Integer.parseInt("1") ){
